...
const InvoiceDetailsLayoutImproved = () => {
  ...
  const handleStatusChange = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_API_BASE_URL || 'http://localhost:5000/api'}/po-invoices/${id}`, {
        method: 'PUT',
        headers: {
    ...
    const fetchInvoice = async () => {
      try {
        ...
        if (isPOInvoice) {
          ...
          response = await fetch(`${process.env.REACT_APP_API_BASE_URL || 'http://localhost:5000/api'}/po-invoices/${id}`);
        } else {
          // Fetch regular invoice
          response = await fetch(`${process.env.REACT_APP_API_BASE_URL || 'http://localhost:5000/api'}/invoices/${id}`);
        }
